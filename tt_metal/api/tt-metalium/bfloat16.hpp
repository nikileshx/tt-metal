...
inline void print_vec_of_uint32_as_packed_bfloat16(
...
inline void print_vec_of_bfloat16(
...
inline void print_vec(
    const std::vector<uint32_t>& vec, int num_tiles, std::string name = "", int tile_print_offset = 0) {
    int idx = 0;
    for (int i = 0; i < num_tiles; i++) {
        std::cout << name << " tile " << i + tile_print_offset << std::endl;
        for (int j = 0; j < 32; j++) {
...
inline bfloat16 bfloat16_identity_transform(const bfloat16& input) { return input; }
